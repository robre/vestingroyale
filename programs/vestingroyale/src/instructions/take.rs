...
pub struct Take<'info> {
    ...
    #[account(mut, has_one = vesting_pool, seeds = [b"vestingroyale", initializer.key().as_ref(), mint.key().as_ref()], bump)]
...
impl Take<'_> {
    pub fn handle(ctx: Context<Self>, args: TakeArgs) -> Result<()> {
        ...
        msg!("ok Allocation is: {}", allocation,);
